...
const columns = (navigate, deleteEntity, displayUniqueName) => [
    {
        ...
        renderCell: (params) => (
            <>
                <IconButton
                    color="primary"
                    onClick={() => navigate(`/update-Mplogilityerrorlog/${params.row.recid}`, { state: { text: displayUniqueName } })}
                >
                    <Edit />
                </IconButton>
                <IconButton
...
const Mplogilityerrorlog = () => {
    ...
    return (
        <Container>
            <h2>{displayUniqueName || 'Mplogilityerrorlog Table'}</h2>
            <Button
                variant="contained"
                color="primary"
                startIcon={<Add />}
                onClick={() => navigate('/add-Mplogilityerrorlog', { state: { text: displayUniqueName } })}
                style={{ marginBottom: 10 }}
            >
                Add Mplogilityerrorlog
            </Button>
...
export default Mplogilityerrorlog;
