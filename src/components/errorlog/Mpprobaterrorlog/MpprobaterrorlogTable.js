...
const columns = (navigate, deleteEntity, displayUniqueName) => [
    {
        ...
        renderCell: (params) => (
            <>
                <IconButton
                    color="primary"
                    onClick={() => navigate(`/update-Mpprobaterrorlog/${params.row.recid}`, { state: { text: displayUniqueName } })}
                >
                    <Edit />
                </IconButton>
                <IconButton
...
const Mpprobaterrorlog = () => {
    ...
    return (
        <Container>
            <h2>{displayUniqueName || 'Mpprobaterrorlog Table'}</h2>
            <Button
                variant="contained"
                color="primary"
                startIcon={<Add />}
                onClick={() => navigate('/add-Mpprobaterrorlog', { state: { text: displayUniqueName } })}
                style={{ marginBottom: 10 }}
            >
                Add Mpprobaterrorlog
            </Button>
...
export default Mpprobaterrorlog;
