...
const columns = (navigate, deleteEntity, displayUniqueName) => [
    {
        ...
        renderCell: (params) => (
            <>
                <IconButton
                    color="primary"
                    onClick={() => navigate(`/update-Mpblujayerrorlog/${params.row.recid}`, { state: { text: displayUniqueName } })}
                >
                    <Edit />
                </IconButton>
                <IconButton
...
const Mpblujayerrorlog = () => {
    ...
    return (
        <Container>
            <h2>{displayUniqueName || 'Mpblujayerrorlog Table'}</h2>
            <Button
                variant="contained"
                color="primary"
                startIcon={<Add />}
                onClick={() => navigate('/add-Mpblujayerrorlog', { state: { text: displayUniqueName } })}
                style={{ marginBottom: 10 }}
            >
                Add Mpblujayerrorlog
            </Button>
...
export default Mpblujayerrorlog;
