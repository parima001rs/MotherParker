...
const columns = (navigate, deleteEntity, displayUniqueName) => [
    {
        ...
        renderCell: (params) => (
            <>
                <IconButton
                    color="primary"
                    onClick={() => navigate(`/update-Mpaveerrorlog/${params.row.recid}`, { state: { text: displayUniqueName } })}
                >
                    <Edit />
                </IconButton>
                <IconButton
...
const Mpaveerrorlog = () => {
    ...
    return (
        <Container>
            <h2>{displayUniqueName || 'Mpaveerrorlog Table'}</h2>
            <Button
                variant="contained"
                color="primary"
                startIcon={<Add />}
                onClick={() => navigate('/add-Mpaveerrorlog', { state: { text: displayUniqueName } })}
