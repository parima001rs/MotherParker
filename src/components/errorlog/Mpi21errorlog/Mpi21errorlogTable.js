...
const columns = (navigate, deleteEntity, displayUniqueName) => [
    {
        ...
        renderCell: (params) => (
            <>
                <IconButton
                    color="primary"
                    onClick={() => navigate(`/update-Mpi21errorlog/${params.row.recid}`, { state: { text: displayUniqueName } })}
                >
                    <Edit />
                </IconButton>
                <IconButton
...
const Mpi21errorlog = () => {
    ...
    return (
        <Container>
            <h2>{displayUniqueName || 'Mpi21errorlog Table'}</h2>
            <Button
                variant="contained"
                color="primary"
                startIcon={<Add />}
                onClick={() => navigate('/add-Mpi21errorlog', { state: { text: displayUniqueName } })}
