...
const columns = (navigate, deleteEntity, displayUniqueName) => [
    {
        ...
        renderCell: (params) => (
            <>
                <IconButton
                    color="primary"
                    onClick={() => navigate(`/update-Mphavierrorlog/${params.row.recid}`, { state: { text: displayUniqueName } })}
                >
                    <Edit />
                </IconButton>
                <IconButton
...
const Mphavierrorlog = () => {
    ...
    return (
        <Container>
            <h2>{displayUniqueName || 'Mphavierrorlog Table'}</h2>
            <Button
                variant="contained"
                color="primary"
                startIcon={<Add />}
                onClick={() => navigate('/add-Mphavierrorlog', { state: { text: displayUniqueName } })}
