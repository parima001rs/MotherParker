...
const columns = (navigate, handleDelete,displayUniqueName) => [
    {
        field: 'actions',
        headerName: 'Actions',
        width: 100,
        renderCell: (params) => (
            <>
                <IconButton
                    color="primary"
                    onClick={() => navigate(`/update-Mpinventtranstracking_Exporttransferords/${params.row.recid}`, { state: { text: displayUniqueName } })}
                >
                    <Edit />
                </IconButton>
                <IconButton
                    color="secondary"
                    onClick={() => handleDelete(params.row.recid)}
                >
                    <Delete />
                </IconButton>
            </>
    ...
    { field: 'inventtrans', headerName: 'Inventtrans No', width: 150 },

    
];

const Mpinventtranstracking_ExporttransferordsTable = () => {
    ...
    useEffect(() => {
        if (entities && entities.length) {
            setLoading(false); // Stop loading when entities are fetched
        }
    }, [entities]);
    ...
    return (
        <Container>
            ...
            {/* <Button
                ...
                color="primary"
                startIcon={<Add />}
                onClick={() => navigate('/add-Mpinventtranstracking_Exporttransferords', { state: { text: displayUniqueName } })}
            >
                Add Mpinventtranstracking_Exporttransferords
            </Button> */}
...
export default Mpinventtranstracking_ExporttransferordsTable;
