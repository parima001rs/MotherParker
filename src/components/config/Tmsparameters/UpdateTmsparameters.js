...
const UpdateTmsparameters = () => {
    const { recid } = useParams(); // Assuming 'recid' is the primary key for this entity type
    const { entities, updateEntity, setType } = useGenericContext();
    const Tmsparameters = entities.find((u) => u.recid === parseInt(recid));
    ...
    useEffect(() => {
        ...
        if (Tmsparameters) {
            Object.keys(Tmsparameters).forEach(key => setValue(key, Tmsparameters[key]));
        } else {
            console.error('Entity not found');
            //navigate('/');
        }
    }, [setType, Tmsparameters, setValue, navigate]);
...
export default UpdateTmsparameters;
