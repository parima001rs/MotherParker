...
const MPI21_PURCHASEGOODSLINESTable = (props) => {
    ...
    const fetchLineData = async (recid) => {
        const response = await axios.get(process.env.REACT_APP_API_URL + '/api/MPI21_PURCHASEGOODSLINES/GetByParent/' + recid);
...
const columns = [
    {
        ...
        renderCell: (params) => (
            <>
                {/* <IconButton
                    color="primary"
                    onClick={() => navigate(`/update-MPI21_PURCHASEGOODSLINES/${params.row.recid}`)}
                >
                    <Edit />
                </IconButton> */}
                <IconButton
    ...
    return (
        <Container>
            <h2>MPI21_PURCHASEGOODSLINES Table</h2>
            <Button style={{ marginBottom: 10 }}
                variant="contained"
                color="primary"
                startIcon={<Add />}
                onClick={() => navigate('/add-MPI21_PURCHASEGOODSLINES')}
            >
...
export default MPI21_PURCHASEGOODSLINESTable;
