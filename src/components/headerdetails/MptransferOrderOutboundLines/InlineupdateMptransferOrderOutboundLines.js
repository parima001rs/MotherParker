...
const MptransferOrderOutboundLineTable = (props) => {
    ...
    const fetchLineData = async (recid) => {
        const response = await axios.get(process.env.REACT_APP_API_URL + '/api/MptransferOrderOutboundLine/GetByParent/' + recid);
...
const columns = [
    ...
    return (
        <Container>
            <h2>MptransferOrderOutboundLine Table</h2>
            <Button style={{ marginBottom: 10 }}
                variant="contained"
                color="primary"
                startIcon={<Add />}
                onClick={() => navigate('/add-MptransferOrderOutboundLine')}
            >
...
export default MptransferOrderOutboundLineTable;
